...
Both services have systemd units (`qwen3-tts-backend.service`, `qwen3-tts-frontend.service`) that require sudo. For manual restarts without sudo:

```bash
# Frontend — PM2 (must rm -rf .next before build to avoid stale chunks)
cd frontend && rm -rf .next && npm run build
pm2 delete frontend && pm2 start "npm start" --name frontend --cwd /var/www/qwen3-tts/frontend

# Backend — start.sh exports API keys and runs gunicorn (1 worker, uvicorn, 900s timeout)
# Kill existing first:  lsof -ti:4728 | xargs kill -9
...
Nginx proxies with `proxy_buffering off` for SSE routes. Max upload 50MB. Timeouts 300s for long TTS/ASR operations. Cloudflare real-IP trust configured. Static assets under `_next/static/` get long-lived cache headers.
...
5. TTS generation (ElevenLabs cloud or Qwen3 local GPU)
    ↓
6. Infographic generation (Gemini 2.5 Flash image)
```
...
- **main.py** — FastAPI app. All long-running endpoints (TTS, ASR, LLM, infographic) use SSE via `StreamingResponse` + `_sse()` helper. Routes:
  ...
  - Infographic: `POST /api/generate-infographic`, `GET /api/infographics/{filename}` (Gemini 2.5 Flash image generation)
...
Single-page app in **page.tsx** (~2200 lines). Seven tabs in studio view:
...
6. **인포그래픽 (Infographic)** — Generate life-journey infographic via Gemini 2.5 Flash image. Combines prompt template with LLM-rewritten text from Source tab. Download generated image.
7. **설정 (Settings)** — TTS engine and LLM model selection
...
- `artifacts/` — Project artifacts (transcript/edited/rewritten text files)
- `infographics/` — Generated infographic PNG images
...
- Four API keys required in production: `GROQ_API_KEY` (ASR + Groq LLMs), `ANTHROPIC_API_KEY` (Claude), `ELEVENLABS_API_KEY` (cloud TTS), `GOOGLE_API_KEY` (Gemini infographic generation). All set in `backend/start.sh` and the systemd service file.
